...
- 2개의 값을 입력받는 창과 버튼이 있는 ㅗ하면을 출력
- 값을 입력하고 버튼을 ㅜ늘면 POST로 전송
- 서버에서는 값을 더한 후 결과값을 JSP에게 request scopeㄹ ㅗ전달하여 출력
...
##### web.xml 파일에서 설정
- xml spring 설 ㅣㅇㄺ어들이도록 DispatcherServlet 설정
...
@Configuration
@EnableWebMvc : 웹에 필요한 빈을 자동으로 설정해준다.
